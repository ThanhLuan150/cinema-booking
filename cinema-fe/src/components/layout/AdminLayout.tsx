    ...
    <nav onClick={() => setIsNavOpen(false)} className="mt-8 flex flex-col gap-1 text-sm">
      ...
      {isEmployee ? (
        ...
        <>
          ...
          {hasPermission('signage.read') && (
            <NavLink to={ROUTES.ownerSignage} className={navLinkClass}>
              <ion-icon name="tv" />
              {t('adminLayout.nav.signage')}
            </NavLink>
          )}
          {hasPermission('parking.read') && (
          ...
          {hasPermission('campaign.read') && (
            <NavLink to={ROUTES.ownerCampaigns} className={navLinkClass}>
              <ion-icon name="rocket" />
              {t('adminLayout.nav.campaigns')}
            </NavLink>
          )}
