...
import { usePostCinemaReply } from '../hooks/usePostCinemaReply';
import { useReactToCinemaReview } from '../hooks/useReactToCinemaReview';
...
const CinemaReviews = () => {
  ...
  const postReplyMutation = usePostCinemaReply();
  const reactMutation = useReactToCinemaReview(id);
  ...
  const handleReply = async (parentId: number, comment: string) => {
    ...
    try {
      await postReplyMutation.mutateAsync({ cinema_id: Number(id), parent_id: parentId, comment });
    } catch (error) {
