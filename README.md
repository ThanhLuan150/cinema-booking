...
Any staff member taking cash (Box Office / Counter Sale / Combo Orders) must have an **open Cashier Shift** — one open shift per employee, enforced by a unique index — and every payment/combo-order/refund they process while it's open is tied to that shift; closing it reconciles expected vs. counted cash. Separately, **Shifts** (a branch's named work shifts, e.g. "Morning") and **Shift Assignments** (which employee works which shift on which date) let a Branch Admin build a staffing roster; each employee sees only their own assignments. An assignment records the **Position** the shift is worked as (defaults to the employee's own; must be an active Position, else `400 INVALID_POSITION`). The backend refuses conflicts: an inactive employee (`EMPLOYEE_NOT_ACTIVE`), a shift from another branch (`BRANCH_MISMATCH`), the same shift twice on a date (`DUPLICATE_ASSIGNMENT`), and any time overlap with the employee's other live assignments (`409 SHIFT_OVERLAP`, comparing real start/end instants so an overnight shift is caught against the next morning; back-to-back shifts are fine; cancelled ones don't count). Reactivating a cancelled assignment re-runs every check, and a post-write re-check backs out the later of two concurrent overlapping writes. A Branch Admin manages only their own branch; a Super Admin can list every branch by omitting `branchId`. Changes are pushed live: `shift:updated` refreshes the branch roster board (and Super Admin), and a separate `myShift:updated` goes only to the employee concerned's account room to raise a "your shift changed" toast — a different event name on purpose, since that employee also sits in the branch room and one event name must travel down one channel only. FE: `/CashierShifts` (staff), `/OwnerShifts` + `/OwnerShifts/Assignments` (Branch Admin), `/EmployeeMySchedule` (an employee's own schedule).
...
### 6.36 F&B Recipes & Ingredients

An **Ingredient** is not a new model: it is a per-branch **Inventory** record (§6.19) that is *not* linked to a sellable item (`combo_id: null`) — name, SKU, unit, stock quantity, cost price and minimum stock are exactly the Inventory fields, so ingredients are stocked, imported (also through Purchase Orders, §6.35), adjusted and wasted like any other stock and alert when low. A **Recipe** belongs to a **Product** — a FOOD/BEVERAGE `Combo` item (a COMBO bundle is made of the items it contains) — and lists `ingredients: [{ inventory_id, quantity }]`, the quantity **per one portion in the ingredient's own unit**. One recipe per product (`409 RECIPE_ALREADY_EXISTS`); every ingredient must exist in **the product's own branch** (`400 INGREDIENT_BRANCH_MISMATCH`) and be raw stock (`400 INGREDIENT_NOT_ALLOWED` for a record linked to a product).

**Selling deducts the ingredients.** Large Popcorn = Corn 150 g + Butter 20 g + Salt 5 g, so selling one takes Corn −150, Butter −20, Salt −5 (three `SALE` history rows, `ref_code` `CO-…:ING<inventory_id>`; quantity sold scales every line; a bundle expands to its components first). The demand of every product in one sale is **summed per ingredient before anything is checked**, so two items that both use butter are tested against the butter shelf together. A product **with a recipe is stock-limited by its ingredients only** — the recipe *replaces* direct stock tracking of that product (the two are mutually exclusive on write: `409 PRODUCT_ALREADY_TRACKED` / `409 PRODUCT_HAS_RECIPE`) so it is never deducted twice. A product with neither is not stock-limited, as before.

**No selling without ingredients, no oversell.** It reuses the Ticket 45 no-oversell path unchanged in shape: creating an order (and pricing a booking's combos) fails fast with `409 INSUFFICIENT_STOCK` naming the short ingredient (`ingredient: true`, `unit`, `product_ids` in `shortages[]`); the authoritative check is at payment, where each ingredient is decremented with **one atomic guarded update** (`quantity >= n` in the filter) — of N concurrent sales for the last portions exactly the affordable ones succeed, none can push stock negative, and a sale that comes up short partway is undone (stock back, ledger rows withdrawn, order stays `PENDING`). Retries and double-pays deduct once (unique ledger key); cancelling a paid order restocks exactly what was taken. An already-paid booking's combos clamp at zero instead of refusing. A recipe whose ingredient record vanished cannot be sold (fail closed). Quantities are fractional (0.1 kg) and snapped to 6 decimals server-side, so `3 × 0.1` is exactly `0.3` and 0.3 kg covers 3 portions of 0.1.

**Calculation** (`utils/recipeCalculation.js`, pure): per-portion `cost_per_portion = Σ quantity × cost_price`, `margin`/`margin_percent` against the product's price, and `max_servings` = whole portions the current stock supports (the scarcest ingredient decides; `limiting_ingredient_id`). `GET /api/recipes/:id/availability?servings=N` answers "can N portions be made?" with per-ingredient requirements and shortages.

**Access.** `recipe.read` / `recipe.manage`: BRANCH for a Branch Admin (own branches; another branch's recipes are `403`), ALL for the Super Admin. **An Employee can change a recipe only if their Position was granted `recipe.manage`** and they are actively staffed at that recipe's branch — no default Position holds `recipe.manage` **or** `recipe.read` (add `recipe.read` to `FNB_STAFF` in `seedPositions.js` to let prep staff see what to make; a floor Employee still *sells* recipe products without either). Guards: an ingredient in a recipe cannot be deleted (`409 INVENTORY_IN_RECIPE`), re-linked to a product (`409 INGREDIENT_IN_RECIPE`) or have its unit changed (`409 INGREDIENT_UNIT_LOCKED` — its quantities would silently change meaning); a product with a recipe cannot become a COMBO; deleting a product deletes its recipe. Changes are audited (`RECIPE_CREATED/UPDATED/DELETED`) and broadcast (`recipe:updated`; stock moves also refresh recipe availability). FE: `/OwnerRecipes` (list with cost/margin/portions, detail with per-ingredient stock and a "N portions?" check, create/edit with a live cost and portion preview). **Run `npm run seed` (RBAC + positions) against the target DB** so `recipe.read`/`recipe.manage` exist.
