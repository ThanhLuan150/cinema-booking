...
describe('employee.repository', () => {
  it('findActiveByAccountAndCinema finds an active match', async () => {
    await Employee.create({ id: 1, account_id: 10, cinema_id: 1, status: 1 });
  ...
  it('findActiveByAccountAndCinema returns null for a deactivated employee', async () => {
    await Employee.create({ id: 1, account_id: 10, cinema_id: 1, status: 0 });
  ...
  it('findAll paginates and filters by cinema_id', async () => {
    await Employee.create([
      { id: 1, account_id: 10, cinema_id: 1 },
      { id: 2, account_id: 11, cinema_id: 1 },
      { id: 3, account_id: 12, cinema_id: 2 },
    ]);
  ...
  it('create persists a new employee', async () => {
    const employee = await employeeRepository.create({ id: 1, account_id: 10, cinema_id: 1, position: 'Cashier' });
